...
roukou									<a href = "{$smarty.server.PHP_SELF}?ctg=comments&view_unit={$smarty.get.view_unit}&edit={$T_COMMENTS[comments_list].id}&popup=1", onclick = "eF_js_showDivPopup(event, '{$smarty.const._EDITCOMMENT}', 1)" target = "POPUP_FRAME"><img class = "handle" src = "images/16x16/edit.png" alt = "{$smarty.const._CORRECTION}" title = "{$smarty.const._CORRECTION}"/></a>&nbsp;
								{/if}
