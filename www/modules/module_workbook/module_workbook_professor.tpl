...
{foreach name = "students_loop" key = "login" item = "student" from = $T_WORKBOOK_STUDENTS}
	<tr id="row_{$student.login}" class="{cycle values = "oddRowColor, evenRowColor"}">
		...
		<td class="centerAlign"><a href="{$T_WORKBOOK_BASEURL}&preview_workbook=1&student={$student.login}&popup=1" onclick="eF_js_showDivPopup(event, '{$smarty.const._PREVIEW}', 3)" target="POPUP_FRAME"><img src="{$T_WORKBOOK_BASELINK|cat:'images/info.png'}" alt="{$smarty.const._PREVIEW}" title="{$smarty.const._PREVIEW}" style="vertical-align:middle" /></a></td>
	</tr>
{foreachelse}
...
{/if}
			<img src="{$T_WORKBOOK_BASELINK|cat:'images/move.png'}" alt="{$smarty.const._MOVE}" title="{$smarty.const._MOVE}" style="vertical-align:middle" />&nbsp;<a href="{$T_WORKBOOK_BASEURL}&move_item={$item.id}&popup=1" onclick="eF_js_showDivPopup(event, '{$smarty.const._WORKBOOK_MOVE_ITEM}', 0)" target="POPUP_FRAME">{$smarty.const._MOVE}</a>
			&nbsp;<div class="actions_separator"></div>&nbsp;
