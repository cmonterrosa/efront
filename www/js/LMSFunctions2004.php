...
if ((!$entry_info && sizeof($result) > 0) || $entry_info == 'resume') {
...
/*
 * An API Implementation object name changed from APBKMEI to API_1484_11
*/
...
function myGetValues(property)
{
	...
	if (!checkState() && str_split[2] != 'request_valid') {
...
function mySetValues(property, value)   //throw new myError(0);
{
	...
	if (!checkState() &&  str_split[2] != 'request') {
    ...
    else
    {
		try {
			...
			*/
            ...
            {
				if ((mykey != 'session_time' || parameter == 'finish') && document.getElementById(mykey)) {
					if (commitArray[mykey]==null || (typeof commitArray[mykey]) == 'undefined') {
						document.getElementById(mykey).disabled = true;
					} else {
						document.getElementById(mykey).disabled = false;
...
function checkDataType(parameter, data_model, type)
{
    switch (data_model) {
    	...
    	case 'CMIString64000':
            return match = /^.{0,64000}$/mi.test(parameter);              //Any character string with length at most 64000
			break;

		case 'characterstring':
			return match = /^.{0,1000}$/mi.test(parameter);              //Any character string with length at most 1000
			break;
